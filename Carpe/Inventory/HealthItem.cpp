...
{
	Health = 50;
	InventoryType = EInventoryType::Health;
}

bool AHealthItem::CanBePickedUp(class ACharacterBase* TestPawn) const
{
	return TestPawn && (TestPawn->Health < TestPawn->GetMaxHealth());
}

void AHealthItem::GivePickupTo(class ACharacterBase* Pawn)
{
	if (Pawn)
	{
