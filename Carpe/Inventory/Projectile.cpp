...
AProjectile::AProjectile(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	CollisionComp->SetCollisionObjectType(COLLISION_PROJECTILE2);
...
void AProjectile::PostInitializeComponents()
{
	...
	AWeapon_Projectile* OwnerWeapon = Cast<AWeapon_Projectile>(GetOwner());
	if (OwnerWeapon)
	{
...
void AProjectile::OnRep_Exploded()
{
	...
	if (!GetWorld()->LineTraceSingleByChannel(Impact, StartTrace, EndTrace, COLLISION_PROJECTILE2, FCollisionQueryParams(TEXT("ProjClient"), true, Instigator)))
	{
		// failsafe
