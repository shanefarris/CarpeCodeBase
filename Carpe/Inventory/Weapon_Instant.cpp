...
void AWeapon_Instant::FireWeapon()
{
	...
	if (WeaponTrace(StartTrace, EndTrace, HitResults))
	{
		for (auto hit : HitResults)
		{
		}
		ProcessInstantHit(HitResults[0], StartTrace, ShootDir, RandomSeed, CurrentSpread);
	}
...
bool AWeapon_Instant::ServerNotifyHit_Validate(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	return true;
}

void AWeapon_Instant::ServerNotifyHit_Implementation(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	...
	if (Instigator && (Impact.GetActor() || Impact.bBlockingHit))
	{
		...
		{
			if (CurrentState != EInventoryState::Idle)
			{
				...
				else
				{
					...
					{
						UE_LOG(LogCore, Log, TEXT("%s Rejected client side hit of %s (outside bounding box tolerance)"), *GetNameSafe(this), *GetNameSafe(Impact.GetActor()));
					}
				}
			}
		}
		...
		{
			UE_LOG(LogCore, Log, TEXT("%s Rejected client side hit of %s (facing too far from the hit direction)"), *GetNameSafe(this), *GetNameSafe(Impact.GetActor()));
		}
		else
		{
			UE_LOG(LogCore, Log, TEXT("%s Rejected client side hit of %s"), *GetNameSafe(this), *GetNameSafe(Impact.GetActor()));
		}
	}
}
...
void AWeapon_Instant::DealDamage(const FHitResult& Impact, const FVector& ShootDir)
{
	...
	if (Impact.BoneName == FName("Neck_01") || Impact.BoneName == FName("Head"))
	{
		PointDmg.Damage *= HeadShotMultiplyer;
	}
...
void AWeapon_Instant::OnBurstFinished()
{
...
void AWeapon_Instant::SimulateInstantHit(const FVector& ShotOrigin, int32 RandomSeed, float ReticleSpread)
{
	FRandomStream WeaponRandomStream(RandomSeed);
...
void AWeapon_Instant::SpawnImpactEffects(const FHitResult& Impact)
{
	if (ImpactTemplate && Impact.bBlockingHit)
	{
		...
		if (!Impact.Component.IsValid())
		{
			...
			WeaponTrace(StartTrace, EndTrace, UseImpact);
		}
