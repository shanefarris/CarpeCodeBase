...
UCoreGameEngine::UCoreGameEngine(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UCoreGameEngine::Init(IEngineLoop* InEngineLoop)
{
...
void UCoreGameEngine::HandleNetworkFailure(UWorld *World, UNetDriver *NetDriver, ENetworkFailure::Type FailureType, const FString& ErrorString)
{
		...
		//if (World)
		{
			...
			if (NetDriver)
			{
				switch (FailureType)
				{
					...
					{
						//UCoreGameInstance* const ShooterInstance = Cast<UCoreGameInstance>(GameInstance);
						...
						//	// this is the case
						//	ShooterInstance->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), CoreGameInstanceState::MainMenu, false );
						//}
						break;
					}
					case ENetworkFailure::PendingConnectionFailure:						
					{
						//UCoreGameInstance* const GI = Cast<UCoreGameInstance>(GameInstance);
						...
						//	// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
						//	// this is the case
						//	GI->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), CoreGameInstanceState::MainMenu, false );
						//}
						break;
					}
					...
					case ENetworkFailure::ConnectionTimeout:
					{
						...
						//	// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
						//	// this is the case
						//	GI->ShowMessageThenGotoState( ReturnReason, OKButton, FText::GetEmpty(), CoreGameInstanceState::MainMenu, false );
						//}
						break;
					}
