	...
	if (URL == TEXT("/index.html?scoreboard"))
	{
		...
		if (GameEngine)
		{
			...
			{			
				...
				{
					// get the shoter game
					ACoreGameState* const GameState = Cast<ACoreGameState>(Player->PlayerController->GetWorld()->GetGameState());


					RankedPlayerMap2 Players;
...
static void ExtendedSaveGameInfoDelegate(const TCHAR* SaveName, const EGameDelegates_SaveGame Key, FString& Value)
{
	...
	switch(Key)
	{
		...
		case EGameDelegates_SaveGame::SubTitle:
			Value = TEXT("The Survivalening");
			break;
		case EGameDelegates_SaveGame::Detail:
			Value = TEXT("Survival User Settings");
			break;
		default:
			break;
	}
