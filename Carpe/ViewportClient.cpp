...
UViewportClient::UViewportClient(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetSuppressTransitionMessage(true);
}

void UViewportClient::NotifyPlayerAdded(int32 PlayerIndex, ULocalPlayer* AddedPlayer)
{
	Super::NotifyPlayerAdded(PlayerIndex, AddedPlayer);

 	UGameLocalPlayer* const ShooterLP = Cast<UGameLocalPlayer>(AddedPlayer);
 	if (ShooterLP)
 	{
...
void UViewportClient::AddViewportWidgetContent( TSharedRef<class SWidget> ViewportContent, const int32 ZOrder )
{
	//UE_LOG( LogPlayerManagement, Log, TEXT( "UViewportClient::AddViewportWidgetContent: %p" ), &ViewportContent.Get() );
...
void UViewportClient::RemoveViewportWidgetContent( TSharedRef<class SWidget> ViewportContent )
{
	UE_LOG( LogPlayerManagement, Log, TEXT( "UViewportClient::RemoveViewportWidgetContent: %p" ), &ViewportContent.Get()  );
...
void UViewportClient::HideExistingWidgets()
{
...
void UViewportClient::ShowExistingWidgets()
{
...
void UViewportClient::ShowDialog(TWeakObjectPtr<ULocalPlayer> PlayerOwner, EDialogType::Type DialogType, const FText& Message, const FText& Confirm, const FText& Cancel, const FOnClicked& OnConfirm, const FOnClicked& OnCancel)
{
	//UE_LOG( LogPlayerManagement, Log, TEXT( "UViewportClient::ShowDialog..." ) );
...
void UViewportClient::HideDialog()
{
	//UE_LOG( LogPlayerManagement, Log, TEXT( "UViewportClient::HideDialog. DialogWidget: %p, OldFocusWidget: %p" ), DialogWidget.Get(), OldFocusWidget.Get() );
...
void UViewportClient::ShowLoadingScreen()
{
	//if ( LoadingScreenWidget.IsValid() )
	//{
	//	return;
...
void UViewportClient::HideLoadingScreen()
{
	//if ( !LoadingScreenWidget.IsValid() )
	//{
	//	return;
...
//EDialogType::Type UViewportClient::GetDialogType() const
//{
//	return (DialogWidget.IsValid() ? DialogWidget->DialogType : EDialogType::None);
//}

//TWeakObjectPtr<ULocalPlayer> UViewportClient::GetDialogOwner() const
//{
//	return (DialogWidget.IsValid() ? DialogWidget->PlayerOwner : nullptr);
//}

void UViewportClient::Tick(float DeltaSeconds)
{
...
#if WITH_EDITOR
void UViewportClient::DrawTransition(UCanvas* Canvas)
{
